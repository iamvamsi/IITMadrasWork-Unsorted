...
asm volatile ("spin_lock:\n\t"
		      ...
		      "movl $1,%%eax\n\t"        //Setting the EAX register to 1 

		      "lock xchg %0,%%eax\n\t"  //Atomically exchanging the values stored in Varaible Lock and eax register leading to store always 1 in Lock varaible.

		      "test %%eax,%%eax\n\t"	//If EAX is 0, then the lock was unlocked and we just locked it. Otherwise, EAX is 1 and we didn't acquire the lock and sets ZERO flag if EAX is zero.

		      "jnz spin_lock"           //Jump back to Spin_lock if ZERO flag was not set.

		      :

		      :"m"(Lock)//A memory operand is allowed with any type of address that the machine supports in general.

		      :"%eax"  //The EAX register is used in the code.
...
asm volatile (
		      "movl $0,%%eax\n"	       //Setting the EAX register to 0

		      "lock xchg %%eax,%0\n"  //Atomically exchange the values in Varaible Lock and eax registers.
...
int main(){
	...
	time=diff(start,end);
	//}
	cout<<dll_size<<"\t"<<MAX_THREADS<<"\t"<<time/1000000000<<endl;
